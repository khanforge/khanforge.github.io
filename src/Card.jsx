...
const Card = (props) => {
    ...
    return (
        <div id={`project-card-${project.id}`} className='mt-5 h-auto min-h-[10rem] w-[291px] bg-slate-950/50 border-[0.5px] border-violet-900 hover:border-[1px] hover:border-violet-500 rounded-md hover:scale-105 transition-all duration-300  ease-in-out overflow-hidden group'>
            ...
            <div id={`discription-${project.id}`} className='p-2'>
                ...
                <div className="">
                    {project.techStack.map((stack, index) => (
                        <span key={index} className={`text-xs py-[2px] px-1 m-[2px] rounded-sm ${index % 3 == 0 ? 'bg-fuchsia-600' : index % 3 == 1 ? 'bg-emerald-600' : 'bg-orange-500'}`}>{stack}</span>
                    ))}
                </div>
                <div className="text-sm m-1 text-justify text-violet-400 group-hover:text-violet-300 break-words">
